...
void Llm::eraseHistory(size_t begin, size_t end) {
    ...
    // [FIX] The KV cache is now invalid for anything the prompt-cache REUSE
    // path might compare against on the next turn (mLastInputTokens /
    // mCachedPromptText describe the pre-erase state). If they are left
    // populated, the next response() computes a common prefix against stale
    // tokens while all_seq_len is already 0, prefilling the delta at the wrong
    // KV positions (cross-turn contamination: subsequent turns of an erased
    // session bleed into each other). Force a full re-prefill by invalidating
    // the cached prompt state.
